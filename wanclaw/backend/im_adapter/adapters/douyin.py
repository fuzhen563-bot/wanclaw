...
class DouyinAdapter(IMAdapter):
    """抖音电商适配器"""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(PlatformType.DOUYIN, config)
        ...
        if not all([self.client_key, self.client_secret]):
            raise ValueError("抖音配置不完整，需要 client_key, client_secret")

    async def connect(self) -> bool:
        try:
            self._connected = True
            self._stats["last_connected"] = asyncio.get_event_loop().time()
            logger.info("抖音适配器连接成功")
            return True
        except Exception as e:
            logger.error(f"抖音适配器连接失败: {e}")
            return False

    async def disconnect(self):
        self._connected = False
        logger.info("抖音适配器已断开")
        ...
        **kwargs
        ...
        try:
            ...
            return MessageResponse.success_response(
                ...
                chat_id=chat_id,
                message_id=f"douyin_{self._stats['messages_sent']}"
            )
        except Exception as e:
...
AdapterFactory.register(PlatformType.DOUYIN, DouyinAdapter)
