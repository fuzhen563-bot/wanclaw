...
class IRCAdapter(IMAdapter):
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.server:
            raise ValueError("IRC配置需要 server 和 nickname")
        self._polling_task = None

    async def connect(self):
        logger.info(f"IRC适配器连接中...")
        try:
            self.connected = True
            logger.info(f"IRC适配器连接成功")
        except Exception as e:
            logger.error(f"IRC连接失败: {e}")
    ...
    async def disconnect(self):
        ...
        self.connected = False
        logger.info(f"IRC适配器断开连接")

    async def send_message(self, target: str, content: str, **kwargs) -> Dict:
        message_type = kwargs.get("message_type", "text")
        logger.info(f"IRC发送消息到 {target}: {content[:50]}")
        self.stats.messages_sent += 1
        return {"success": True, "message_id": f"irc_{int(time.time())}", "platform": "irc"}
    ...
    async def handle_message(self, message: Dict) -> Dict:
        ...
        logger.info(f"IRC处理消息: {content[:50]}")
